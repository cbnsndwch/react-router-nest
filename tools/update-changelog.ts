...
function generateChangelog(): string {
    ...
    try {
        // Use the custom config to include chore commits
        execSync(
            `npx conventional-changelog -n ./tools/changelog-config.cjs -o ${TEMP_FILE} -r 0 -t ${TAG_PREFIX}`,
            {
